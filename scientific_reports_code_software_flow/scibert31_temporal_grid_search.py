...
if __name__ == "__main__":
    ...
    early_papers = []
    with open('retraction_watch_embeddings30_scibert_early.csv', 'r') as file:
    ...
    with open('semantic_scholar_embeddings30_scibert_early.csv', 'r') as file:
        for i, line in enumerate(file):
            ###replace elements of the abstract with nothing so that it doesn't confuse ChatGPT
            line = line.replace('[','').replace(']','').replace("\n",'')
            if line != '':
                ...
                early_papers.append(paper)
    recent_papers = []
    with open('retraction_watch_embeddings30_scibert_recent.csv', 'r') as file:
        for i, line in enumerate(file):
            ###replace elements of the abstract with nothing so that it doesn't confuse ChatGPT
            line = line.replace('[','').replace(']','').replace("\n",'')
            if line != '':
    ...
    with open('semantic_scholar_embeddings30_scibert_recent.csv', 'r') as file:
        for i, line in enumerate(file):
            ###replace elements of the abstract with nothing so that it doesn't confuse ChatGPT
            line = line.replace('[','').replace(']','').replace("\n",'')
            if line != '':
    ...
    aucpr_best = 0
    for C in [0.1, 1., 10.]:
        print("C", C, datetime.datetime.now())
        classifier = svm.SVC(kernel="linear", class_weight = 'balanced', C=C, probability=True)
        classifier.fit(Xtrain, Ytrain)
        y_pred = classifier.predict_proba(Xtest)[:,1]
