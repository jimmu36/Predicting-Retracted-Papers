###openai32_fraud_error_temporal_grid_search uses the valid/retracted embeddings, but applies them to the fraud/error dataset. The idea is the
###valid/retracted model had more data to train on and might give better performance.
###openai31_temporal_grid_search adds: 1) scaler.fit_transform to Xtest and splits out Xval/Yval from Xtrain/Ytrain and
...
class Node:
    def __init__(self):
        self.embedding = []
        self.num_words = 0
        self.fraud_error = -1   ###retracted = 1, not-retracted = 0
        self.date = ''
    def __repr__(self):
...
if __name__ == "__main__":
    ...
    aucpr_best = 0
    for C in [0.1, 1., 10.]:
        for gamma in [0.00001, 0.0001, 0.001]:
