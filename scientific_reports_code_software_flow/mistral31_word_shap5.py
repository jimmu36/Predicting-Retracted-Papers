####mistral31_word_shap5 takes a file of 1000 previously identified strongly predicted valid papers (<25%) and calculates the shap values for
...
###mistral31_word_shap3 uses strongly predicted valid papers (<25%) to create a dictionary of top words.
...
if __name__ == "__main__":
    ...
    all_indices = []
    with open('valid_1000_indices.csv', 'r') as f:
    ...
    for i, idx in enumerate(sampled_indices):
        ...
        for j, word in enumerate(words):
            ...
            if word in words_shap_values:
                words_shap_values[word].append(values[j])
            else:
                words_shap_values[word] = [values[j]]
    ###words_ave_shap_value = {}
    with open('valid_words_shap_values_100_'+args.shap_100_index+'.csv', 'w') as f:
        writer = csv.writer(f)
